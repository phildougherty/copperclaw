...
//!
//! These wrap the existing `ironclaw run` command into a one-terminal flow.
...
pub const PID_FILE_NAME: &str = "ironclaw.pid";

/// Subdirectory under the data dir for log files.
pub const LOG_DIR_NAME: &str = "logs";

/// Filename for the host log inside `<data_dir>/logs/`.
pub const LOG_FILE_NAME: &str = "ironclaw.log";

/// Internal marker env var: set on the child of `ironclaw start` so the
/// re-exec'd process knows it should run `ironclaw run` instead of
/// re-entering the spawn dance.
pub const DAEMONIZED_ENV: &str = "IRONCLAW_DAEMONIZED";
...
pub enum DaemonError {
    ...
    #[error("ironclaw already running (pid {0}); run `ironclaw status` or `ironclaw stop`")]
    AlreadyRunning(i32),
    /// The host did not bind its socket within [`START_READY_TIMEOUT`].
    #[error(
        "ironclaw start: host did not become ready within {timeout:?}; \
         check {log}"
    )]
    StartTimeout {
        timeout: Duration,
    ...
    #[error("ironclaw not running (stale pid {0})")]
    StalePid(i32),
    /// `--strict` requested but no PID file present.
    #[error("ironclaw not running")]
    NotRunning,
}
...
///
/// The child inherits `IRONCLAW_DAEMONIZED=1` and `IRONCLAW_DATA_DIR`
...
/// `ironclaw start` — spawn detached, wait for ready, write PID file.
///
/// `extra_args` are passed through to the child's `ironclaw run`
...
pub fn cmd_start(
    ...
    // Make sure the data dir exists before we hand off so `IRONCLAW_DATA_DIR`
...
/// `ironclaw stop` — read PID file, send TERM, wait, escalate to KILL.
///
...
/// `ironclaw status` — describe what's running.
///
...
fn count_active_sessions(cfg: &HostConfig) -> Option<usize> {
    ...
    let db = ironclaw_db::central::CentralDb::open(&path).ok()?;
    let rows = ironclaw_db::tables::sessions::list_active(&db).ok()?;
    Some(rows.len())
}

/// Snapshot of `ironclaw status`.
#[derive(Debug, Clone)]
...
impl StatusSnapshot {
    ...
    pub fn render_text(&self) -> String {
        ...
        if self.running {
            s.push_str("ironclaw: running\n");
        } else {
            s.push_str("ironclaw: not running\n");
        }
...
/// `ironclaw logs` — print the last `tail` lines, optionally follow.
///
...
mod tests {
    ...
    fn paths_from_data_dir_layout() {
        let p = DaemonPaths::for_data_dir(
            ...
            Path::new("/tmp/ironclaw/iclaw.sock"),
        );
        assert_eq!(p.pid_file, PathBuf::from("/tmp/ironclaw/ironclaw.pid"));
        assert_eq!(p.log_dir, PathBuf::from("/tmp/ironclaw/logs"));
        assert_eq!(
            p.log_file,
            PathBuf::from("/tmp/ironclaw/logs/ironclaw.log")
        );
    }
    ...
    fn status_snapshot_text_includes_paths() {
        let snap = StatusSnapshot {
            ...
            pid: None,
            uptime: None,
            data_dir: PathBuf::from("/srv/ironclaw"),
            socket: PathBuf::from("/srv/ironclaw/iclaw.sock"),
            log_file: PathBuf::from("/srv/ironclaw/logs/ironclaw.log"),
            active_sessions: None,
        };
        let text = snap.render_text();
        assert!(text.contains("not running"));
        assert!(text.contains("/srv/ironclaw"));
        assert!(text.contains("iclaw.sock"));
    }
