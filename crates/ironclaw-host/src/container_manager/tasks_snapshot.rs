...
use ironclaw_db::tables::tasks::{self, Task, TaskStatus};
...
/// On-disk row shape. Matches `ironclaw_mcp::context::TaskSummary` so the
/// runner can deserialize directly into that type. We don't import
/// `TaskSummary` here because the host crate doesn't depend on
/// `ironclaw-mcp` (same circular-dep rationale as `RunnerConfigForFile`).
