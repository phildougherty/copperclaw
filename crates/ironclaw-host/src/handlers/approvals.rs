...
//!   created — that is a separate operator decision (`iclaw wirings create`).
...
//!   operator knows to run `iclaw groups restart <agent_group_id>`.
...
use ironclaw_db::tables::pending_approvals::ApprovalStatus;
...
/// separate operator decision (`iclaw wirings create --mg <id> --ag <id>`).
fn apply_channel(
    central: &CentralDb,
    row: &pending_approvals::PendingApproval,
) -> Result<Value, ErrorPayload> {
    ...
    Ok(json!({
        ...
        "wiring_hint": format!(
            "messaging group created; wire it to an agent group with \
             `iclaw wirings create --mg {} --ag <ag_id> --engage <mode>`",
            mg.id.as_uuid()
        ),
    }))
}
    ...
    Ok(json!({
        ...
        "rebuild_hint": format!(
            "packages merged into container_configs but NOT rebuilt; \
             run `iclaw groups restart {}` when convenient",
            ag_id.as_uuid()
        ),
    }))
}
...
mod tests {
    ...
    fn seed_ag(db: &CentralDb) -> ironclaw_types::AgentGroupId {
        create_ag(
            db,
            CreateAgentGroup {
                name: "g".into(),
    ...
    fn approve_install_packages_merges_into_container_config() {
        ...
        assert!(hint.contains("iclaw groups restart"));
