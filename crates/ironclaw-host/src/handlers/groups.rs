...
use ironclaw_db::tables::{agent_groups, container_configs, sessions};
use ironclaw_types::ContainerStatus;
...
mod tests {
    ...
    fn restart_stops_running_sessions_but_leaves_already_stopped() {
        use ironclaw_db::tables::sessions::{self as s, CreateSession};
        let db = db();
        let g = make_group(&db, "g");
    ...
    fn restart_unknown_group_is_not_found() {
        let db = db();
        let fake = ironclaw_types::AgentGroupId::new();
