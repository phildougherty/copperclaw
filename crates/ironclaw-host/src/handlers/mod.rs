//! Handlers for every `iclaw` command in [`ironclaw_iclaw::ALL_COMMANDS`].
//!
//! Each handler maps a JSON arg payload onto an `ironclaw-db` table function
...
use ironclaw_iclaw::{Caller, ErrorPayload};
...
/// container-side `iclaw` shim can introspect its own configuration.
...
pub(crate) fn db_err(err: ironclaw_db::DbError) -> ErrorPayload {
    match err {
        ironclaw_db::DbError::NotFound => ErrorPayload::new("not_found", "not found"),
...
mod tests {
    ...
    fn db_err_maps_notfound() {
        let p = db_err(ironclaw_db::DbError::NotFound);
    ...
    fn db_err_maps_other_to_db_error() {
        ...
        let p = db_err(ironclaw_db::DbError::Invariant("x".into()));
