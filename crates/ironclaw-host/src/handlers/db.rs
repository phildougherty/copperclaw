...
//! ## Backup
//!
//! `iclaw db backup <path>` runs a WAL checkpoint (`PRAGMA
...
//! ## Restore
//!
//! `iclaw db restore <path>` refuses to run while the host process is alive
...
/// the database. The operator must stop the host (`systemctl stop ironclaw`
/// or kill the process), then restore by running:
///
///   cp <backup-path> <data-dir>/ironclaw.db
///
/// Once the host is stopped, the restore can also be done by a one-shot
/// `ironclaw migrate` invocation against the restored file to re-apply any
...
pub fn restore(_args: &Value, _central: &CentralDb) -> Result<Value, ErrorPayload> {
    Err(ErrorPayload::new(
        ...
        "db restore cannot run while the host is active: the host holds an open WAL connection \
         ...
         (e.g. `systemctl stop ironclaw`), then copy the backup file over \
         `<data_dir>/ironclaw.db` manually.",
    ))
}
