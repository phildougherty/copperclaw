...
use ironclaw_db::tables::messaging_group_agents;
use ironclaw_iclaw::ErrorPayload;
use ironclaw_types::{AgentGroupId, EngageMode, MessagingGroupId, SessionMode, WiringId};
...
pub fn list(args: &Value, central: &CentralDb) -> Result<Value, ErrorPayload> {
    ...
    let rows = match (filter_mg, filter_ag) {
        ...
        (None, None) => {
            ...
            let mgs = ironclaw_db::tables::messaging_groups::list(central).map_err(db_err)?;
