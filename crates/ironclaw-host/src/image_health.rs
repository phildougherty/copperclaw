...
//! nothing. To prevent that, `ironclaw run` performs a boot-time
...
//! works and `iclaw doctor` is reachable), but the container manager
...
use ironclaw_db::tables::{messages_in, messages_out, sessions};
...
/// `ironclaw_degraded_state` Prometheus gauge.
...
pub enum HealthDegradedReason {
    ...
    /// at `/usr/local/bin/ironclaw-runner`.
...
impl HealthDegradedReason {
    /// The `reason` label value used for the `ironclaw_degraded_state`
    /// gauge.
    #[must_use]
    pub fn metric_label(&self) -> &'static str {
        match self {
            Self::ImageNotFound { .. } => {
                ironclaw_metrics::DEGRADED_REASON_IMAGE_NOT_FOUND
            }
            Self::RunnerBinaryMissing { .. } => {
                ironclaw_metrics::DEGRADED_REASON_RUNNER_BINARY_MISSING
            }
            Self::RunnerBinaryNotExecutable { .. } => {
                ironclaw_metrics::DEGRADED_REASON_RUNNER_BINARY_NOT_EXECUTABLE
            }
            Self::Timeout(_) => ironclaw_metrics::DEGRADED_REASON_HEALTH_CHECK_TIMEOUT,
            Self::Failed(_) => ironclaw_metrics::DEGRADED_REASON_HEALTH_CHECK_FAILED,
        }
    }
}
...
impl ImageProbe for DockerImageProbe {
    ...
    async fn check_binary(&self, tag: &str, path: &str) -> Result<BinaryCheck, String> {
        ...
        //   -rwxr-xr-x 1 root root 1234 May 21 12:00 /usr/local/bin/ironclaw-runner
...
/// sibling of the current executable first, then `/usr/local/bin/ironclaw-runner`,
/// then `$HOME/.local/bin/ironclaw-runner`. Returns the first match.
///
...
pub fn default_host_runner_path() -> Option<PathBuf> {
    ...
    for candidate in ["/usr/local/bin/ironclaw-runner", "/usr/bin/ironclaw-runner"] {
    ...
    if let Some(home) = std::env::var_os("HOME") {
        let candidate = PathBuf::from(home).join(".local/bin/ironclaw-runner");
        if candidate.is_file() {
            return Some(candidate);
        }
    }
...
/// `Some`, a mismatch against the image's `ironclaw.fingerprint`
...
///    write pattern as `ironclaw_runner::run::emit_terminal_failure_apologies`.
/// 3. `ironclaw_degraded_state{reason=<reason>}` is set to 1.
...
pub fn enter_degraded_mode(
    ...
    ironclaw_metrics::set_degraded_state(reason.metric_label());
...
fn emit_degraded_apology(paths: &SessionPaths) -> Result<bool, ironclaw_db::DbError> {
    ...
    let Some(target) = pending
        .iter()
        .find(|m| matches!(m.kind, ironclaw_types::MessageKind::Chat))
    else {
        return Ok(false);
    };
...
pub(crate) mod tests {
    ...
    async fn image_health_passes_when_image_has_runner() {
        ...
        let res = check_image_health(&probe, "ironclaw/session:test", Some("fp-test")).await;
    ...
    async fn image_health_fails_when_runner_binary_absent() {
        ...
        let res = check_image_health(&probe, "ironclaw/session:test", None).await;
        match res {
            Err(HealthDegradedReason::RunnerBinaryMissing { tag, path }) => {
                assert_eq!(tag, "ironclaw/session:test");
    ...
    async fn image_health_warns_on_fingerprint_mismatch() {
        ...
        let res = check_image_health(&probe, "ironclaw/session:test", Some("host-fp")).await;
        assert!(
            res.is_ok(),
    ...
    fn host_runner_fingerprint_returns_none_for_missing_file() {
        let p = Path::new("/definitely/not/a/file/ironclaw-runner");
