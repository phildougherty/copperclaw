...
/// `grep -nE 'serde_json::json!\(\{ ?"[a-z_]+":' crates/ironclaw-runner/src/tools.rs`
/// plus the `usage_report` emitter in `crates/ironclaw-runner/src/run.rs`.
...
async fn install_built_in_modules_mock(ctx: Arc<MockModuleContext>) {
    // Order mirrors `crates/ironclaw-host/src/boot.rs::install_modules`.
...
async fn every_runner_emit_has_a_host_handler() {
    ...
    assert!(
        ...
        "RUNNER EMITS ACTION NAMES WITH NO HOST HANDLER: {unhandled:?}\n\
         ...
         `crates/ironclaw-modules/src/<module>.rs::install`, or add the\n\
         action to `DeliveryService::handle_system`'s inline arms in\n\
         `crates/ironclaw-host-delivery/src/service.rs`. If the missing\n\
         action is registered by a module that exists but is not added to\n\
         `boot::install_modules` in `crates/ironclaw-host/src/boot.rs`,\n\
         wire it up there.\n\
         \n\
         ...
         `crates/ironclaw-runner/src/run.rs::emit_usage_report`."
    );
}
...
fn runner_emit_set_matches_source() {
    ...
    assert!(
        ...
        "runner emit set drifted from source.\n\
         ...
         - crates/ironclaw-runner/src/tools.rs (every `fn apply_*` body)\n\
         - crates/ironclaw-runner/src/run.rs (`fn emit_usage_report` body)"
    );
}
...
fn host_handle_set_matches_inline_arms() {
    let service_src = std::fs::read_to_string(delivery_path("service.rs"))
        .expect("read crates/ironclaw-host-delivery/src/service.rs");
    ...
    assert!(
        ...
        "inline-handler set drifted from `DeliveryService::handle_system`.\n\
         ...
         \n\
         Source file scanned:\n\
         - crates/ironclaw-host-delivery/src/service.rs"
    );
}
...
fn workspace_root() -> PathBuf {
    // CARGO_MANIFEST_DIR points at `crates/ironclaw-host/`; workspace
