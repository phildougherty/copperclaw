...
//!
//! Boots `ironclaw_host::run_host` against a temp install root, mounts a
...
use ironclaw_db::session::{open_inbound, open_inbound_rw_no_mmap, open_outbound, SessionPaths};
use ironclaw_db::tables::{messages_in, session_routing, sessions};
use ironclaw_host::config::ChannelInit;
use ironclaw_host::{run_host, HostConfig};
use ironclaw_iclaw::{run_cli, CallTransport, Caller, ClientError, RunOutput};
use ironclaw_providers::AnthropicProvider;
use ironclaw_runner::{compaction::CompactionCfg, run_loop, RunnerDeps, RunnerToolCtx};
use ironclaw_types::{routing::SessionRouting, AgentGroupId, ChannelType, Effort, SessionId};
use serde_json::json;
...
// purely so `run_host` doesn't fall through to `ironclaw_container_rt::detect()`.
// ---------------------------------------------------------------------------
...
// one turn of `ironclaw_runner::run_loop` against the configured
// wiremock-backed `AnthropicProvider`.
//
// This is the same library-level seam the replay harness uses
// (see crates/ironclaw-host/tests/replay/harness.rs `run_one_turn`).
// ---------------------------------------------------------------------------
...
fn ensure_session_routing(data_dir: &std::path::Path, session: &ironclaw_types::Session) -> Result<()> {
...
async fn run_one_turn(
    ...
    let tool_defs: Vec<ironclaw_providers::ToolDef> = tool_set
        ...
        .collect();
    let tool_map: Arc<std::collections::HashMap<String, Arc<ironclaw_mcp::ToolEntry>>> = Arc::new(
        tool_set
            .into_iter()
...
async fn e2e_chat_round_trip_via_fifo_and_log() {
    ...
    let socket_path = install_root.path().join("iclaw.sock");
...
//
// Drives `iclaw chat` (via `ironclaw_iclaw::run_cli`) with
// `--no-autostart` against a definitely-missing FIFO. Asserts the
// stderr names the FIFO path and hints at `ironclaw start`. No
// wiremock or host needed.
// ---------------------------------------------------------------------------

/// Stub transport that errors on every call. `iclaw chat` is a
...
impl CallTransport for UnreachableTransport {
    async fn call(
        ...
        Err(ClientError::Remote(
            ironclaw_iclaw::ErrorPayload::new(
                "unreachable",
