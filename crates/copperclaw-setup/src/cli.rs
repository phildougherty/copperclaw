...
mod tests {
    ...
    fn cli_parses_list_steps() {
        let cli = Cli::try_parse_from(["copperclaw-setup", "--list-steps"]).unwrap();
    ...
    fn run_from_args_list_steps() {
        let code = run_from_args(["copperclaw-setup", "--list-steps"]).unwrap();
        assert_eq!(code, 0);
    }

    #[test]
    fn run_from_args_generate_unit_stdout() {
        let code = run_from_args(["copperclaw-setup", "--generate-unit", "launchd"]).unwrap();
        assert_eq!(code, 0);
    }
    ...
    fn run_from_args_unknown_unit_errors() {
        let err = run_from_args(["copperclaw-setup", "--generate-unit", "upstart"]).unwrap_err();
        assert!(matches!(err, StepError::Other(_)));
    }
    ...
    fn run_from_args_bad_flag_errors() {
        let err = run_from_args(["copperclaw-setup", "--no-such-flag"]).unwrap_err();
        assert!(matches!(err, StepError::Other(_)));
    }

    #[test]
    fn run_from_args_help_returns_zero() {
        let code = run_from_args(["copperclaw-setup", "--help"]).unwrap();
        assert_eq!(code, 0);
    }

    #[test]
    fn run_from_args_version_returns_zero() {
        let code = run_from_args(["copperclaw-setup", "--version"]).unwrap();
        assert_eq!(code, 0);
    }
}
