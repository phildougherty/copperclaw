...
/// we point them at `cclaw quickstart cli` as the manual fallback.
#[must_use]
pub fn instructions(cfg: &SetupConfig) -> Vec<String> {
    let mut out: Vec<String> = vec![
        "Setup is complete. To start the host:".to_string(),
        "  copperclaw run".to_string(),
        String::new(),
        "(`copperclaw run` auto-discovers the .env in this install root; the".to_string(),
        "companion `cclaw` resolves the same socket without flags.)".to_string(),
    ];
    match cfg.first_channel.as_str() {
        "cli" => {
            ...
            if cfg.quickstart_group_created {
                ...
                out.push("Then, in a second terminal:".to_string());
                out.push("  cclaw quickstart cli --name first".to_string());
                out.push("  cclaw chat".to_string());
                out.push(String::new());
                out.push(
        ...
        other => {
            ...
            out.push(String::new());
            out.push("Run `cclaw doctor` once configured to verify the install.".to_string());
        }
    }
...
mod tests {
    ...
    fn instructions_cli_channel() {
        ...
        // `copperclaw run --data-dir` form). Auto-discovery means no
        // `--env-file` flag. When the quickstart_group step didn't
        // run, we expect a fallback `cclaw quickstart cli` line.
        assert!(out.iter().any(|m| m.trim() == "copperclaw run"));
        assert!(out.iter().any(|m| m.contains("cclaw quickstart cli")));
        // `cclaw doctor` is always referenced in the cli path so
        // first-run users find the diagnostic command without
        // searching docs.
        assert!(out.iter().any(|m| m.contains("cclaw doctor")));
    }
    ...
    fn instructions_after_quickstart_say_chat_directly() {
        ...
        let out = instructions(&cfg);
        assert!(out.iter().any(|m| m.trim() == "copperclaw run"));
        assert!(out.iter().any(|m| m.trim() == "cclaw chat"));
    ...
    fn instructions_mention_service_unit_when_present() {
        let cfg = SetupConfig {
            service_unit_path: PathBuf::from("/x/copperclaw.service"),
            first_channel: "cli".into(),
            ..SetupConfig::default()
        };
        let out = instructions(&cfg);
        assert!(out.iter().any(|m| m.contains("/x/copperclaw.service")));
    }
