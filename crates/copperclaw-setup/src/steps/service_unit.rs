...
//! honored from the headless env var `COPPERCLAW_SETUP_SERVICE_SCOPE`):
//!
...
//! `cclaw.sock` admin socket for up to ~10s so the operator sees a
...
impl Step for ServiceUnitStep {
    ...
    fn run(
        ...
        if scope == ServiceScope::System && !self.runner.is_root() {
            eprintln!(
                "  system scope requires root: re-run `copperclaw-setup` under sudo or \
                 set COPPERCLAW_SETUP_SERVICE_SCOPE=user (falling back to `user` scope)"
            );
        ...
        let template_root = std::env::var_os("COPPERCLAW_TEMPLATE_ROOT").map(PathBuf::from);
        ...
        // the `.env` and execs copperclaw — launchd has no native env-file
        // mechanism. The plist's `ProgramArguments` already points at this
        // wrapper path (see `crates/copperclaw-setup/src/units.rs::render_launchd`).
        ...
        if !enable {
            messages.push(format!(
                "service not enabled (re-run with COPPERCLAW_SETUP_SERVICE_ENABLE=yes), \
                 to do it manually: {}",
        ...
        if came_up {
            messages.push(format!(
                "copperclaw service is running, socket at {}",
                socket_path.display()
            ));
        } else {
            messages.push(format!(
...
pub fn build_command_plan(
    ...
    match (scope, kind) {
        (ServiceScope::System, UnitKind::Systemd) => vec![
            ...
            PlannedCommand::new(
                "systemctl",
                vec!["enable".into(), "--now".into(), "copperclaw.service".into()],
            ),
        ],
...
pub fn diagnostic_hint(kind: UnitKind, runner: &dyn ServiceRunner) -> String {
    match kind {
        UnitKind::Systemd => "journalctl -u copperclaw".to_string(),
        UnitKind::Launchd => format!("launchctl print gui/{}/com.copperclaw.host", runner.uid()),
    }
}
...
pub fn manual_enable_hint(kind: UnitKind, path: &Path) -> String {
    match kind {
        UnitKind::Systemd => {
            "systemctl --user daemon-reload && systemctl --user enable --now copperclaw.service"
                .to_string()
        }
...
/// back to a sensible default. Heuristic: assume `copperclaw` is on
/// PATH at the conventional `/usr/local/bin/copperclaw`. Operators can
/// edit the generated unit if their layout differs.
#[must_use]
pub fn exec_path_from_config(_cfg: &SetupConfig) -> PathBuf {
    PathBuf::from("/usr/local/bin/copperclaw")
}
...
/// mirrors `copperclaw_host::socket::default_socket_path` (intentionally
/// duplicated rather than imported to avoid a cycle through the host
/// crate — both definitions just append `cclaw.sock`).
...
/// initialized; later `cclaw status` calls will catch any deeper
/// breakage.
...
mod tests {
    ...
    fn path_for_scope_system_systemd_is_etc() {
        ...
        .unwrap();
        assert_eq!(p, PathBuf::from("/etc/systemd/system/copperclaw.service"));
    }
    ...
    fn path_for_scope_system_launchd_is_library_daemons() {
        ...
        assert_eq!(
            p,
            PathBuf::from("/Library/LaunchDaemons/com.copperclaw.host.plist")
        );
    }
    ...
    fn build_command_plan_systemd_system_omits_user_flag() {
        ...
        let plan = build_command_plan(
            ServiceScope::System,
            UnitKind::Systemd,
            Path::new("/etc/systemd/system/copperclaw.service"),
            &runner,
        );
        assert_eq!(plan.len(), 2);
        for cmd in &plan {
    ...
    fn build_command_plan_launchd_user_uses_gui_domain() {
        ...
        let plan = build_command_plan(
            ServiceScope::User,
            UnitKind::Launchd,
            Path::new("/h/Library/LaunchAgents/com.copperclaw.host.plist"),
            &runner,
        );
    ...
    fn build_command_plan_launchd_system_uses_system_domain() {
        ...
        let plan = build_command_plan(
            ServiceScope::System,
            UnitKind::Launchd,
            Path::new("/Library/LaunchDaemons/com.copperclaw.host.plist"),
            &runner,
        );
        assert_eq!(plan.len(), 1);
        assert_eq!(plan[0].argv[0], "bootstrap");
    ...
    fn diagnostic_hint_launchd_includes_uid() {
        ...
        assert!(hint.contains("gui/1000/com.copperclaw.host"), "got: {hint}");
    }
    ...
    fn default_socket_path_appends_filename() {
        assert_eq!(
            ...
            PathBuf::from("/srv/iron/cclaw.sock")
        );
    }
    ...
    fn step_print_scope_writes_and_emits_hint() {
        ...
        let dir = tempdir().unwrap();
        let target = dir.path().join("custom/copperclaw.service");
    ...
    fn step_user_scope_runs_systemctl_user_and_polls_socket() {
        ...
        let dir = tempdir().unwrap();
        let target = dir.path().join("copperclaw.service");
        let mut cfg = sample_cfg(dir.path());
        let mut state = SetupState::new();
        let runner = FakeRunner::new().with_socket(true);
        let prompt = Scripted::new()
        ...
        assert!(target.exists());
        let joined = res.messages.join("\n");
        assert!(joined.contains("copperclaw service is running"), "{joined}");
        assert!(joined.contains("cclaw.sock"));
    }
