...
//!
//! Opens `<data_dir>/data/copperclaw.db` via `copperclaw-db`, which runs the
...
impl Step for CentralDbStep {
    ...
    fn description(&self) -> &'static str {
        "Initialise the central copperclaw database"
    }
    ...
    fn run(
        ...
        // schema. Mirrors `copperclaw_host::boot::check_schema_version`
...
///
/// This mirrors `copperclaw_host::boot::check_schema_version`: the wizard
/// must not run against a "future" schema (an older binary touching a
/// DB that was migrated by a newer copperclaw release). Pre-existing
