...
impl Step for DataDirStep {
    ...
    fn description(&self) -> &'static str {
        "Create the copperclaw data directory layout"
    }
    ...
    fn run(
        ...
        if cfg.data_dir.as_os_str().is_empty() {
            let default = default_data_dir().unwrap_or_else(|| PathBuf::from("./copperclaw"));
        ...
        cfg.central_db_path = cfg.data_dir.join("data").join("copperclaw.db");
...
///
/// - Linux: `$XDG_DATA_HOME/copperclaw` (falling back to `~/.local/share/copperclaw`).
/// - macOS: `~/Library/Application Support/copperclaw`.
/// - Other targets: `~/.copperclaw`.
///
...
mod tests {
    ...
    fn default_data_dir_macos() {
        ...
        assert_eq!(
            p,
            PathBuf::from("/home/u/Library/Application Support/copperclaw")
        );
    }
    ...
    fn default_data_dir_linux_fallback() {
        ...
        assert_eq!(p, PathBuf::from("/home/u/.local/share/copperclaw"));
    }

    #[test]
    fn default_data_dir_other_os() {
        let p = default_data_dir_for(Path::new("/h"), "freebsd");
        assert_eq!(p, PathBuf::from("/h/.copperclaw"));
    }
    ...
    fn step_run_uses_default_when_empty() {
        ...
        assert_eq!(cfg.central_db_path, dir.path().join("data/copperclaw.db"));
    }
