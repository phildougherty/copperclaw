...
pub struct EnvReport {
    ...
    pub has_gh: bool,
    /// Whether `cclaw` (the copperclaw CLI agent) is on `PATH`.
    pub has_ncl: bool,
}
...
pub struct SetupConfig {
    /// Root data directory (e.g. `~/.local/share/copperclaw`).
    ...
    /// `cclaw chat` directly; otherwise we point them at
    /// `cclaw quickstart cli` as the manual fallback.
    #[serde(default)]
...
mod tests {
    ...
    fn setup_config_roundtrips_via_json() {
        let c = SetupConfig {
            ...
            central_db_path: PathBuf::from("/tmp/x/data/copperclaw.db"),
            ...
            service_unit_path: PathBuf::from("/tmp/x/copperclaw.service"),
