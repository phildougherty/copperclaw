//! End-to-end integration tests for the `copperclaw-setup` wizard.
//!
...
//! - `COPPERCLAW_CLI_FIFO` / `COPPERCLAW_CLI_LOG` are persisted in the `.env`.
...
fn happy_prompt(dir: &Path) -> Scripted {
    let unit_path = dir.join("copperclaw.service");
    Scripted::new()
        ...
        .with("COPPERCLAW_SETUP_QUICKSTART", "yes")
}
...
fn assert_env_file_contents(dir: &Path) {
    ...
    assert!(
        ...
        ".env should set COPPERCLAW_DATA_DIR={}; body: {env_body}",
        expected_data_dir.display()
    );
    let fifo_path = dir.join("chat.fifo");
    let log_path = dir.join("chat.log");
    assert!(
        env_body.contains(&format!("COPPERCLAW_CLI_FIFO={}\n", fifo_path.display())),
        ".env should set COPPERCLAW_CLI_FIFO={}; body: {env_body}",
        fifo_path.display()
    );
    assert!(
        env_body.contains(&format!("COPPERCLAW_CLI_LOG={}\n", log_path.display())),
        ".env should set COPPERCLAW_CLI_LOG={}; body: {env_body}",
        log_path.display()
    );
    #[cfg(unix)]
    {
...
fn wizard_is_idempotent_when_rerun_against_same_data_dir() {
    ...
    // scratch, which means the `COPPERCLAW_CLI_FIFO` / `COPPERCLAW_CLI_LOG`
    ...
    assert!(
        fifo_lines <= 1,
        "COPPERCLAW_CLI_FIFO should appear at most once; body: {env_body}"
    );
    ...
    assert!(
        log_lines <= 1,
        "COPPERCLAW_CLI_LOG should appear at most once; body: {env_body}"
    );
