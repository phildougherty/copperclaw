...
//! can be called from ironclaw with our own `McpError` type.
//!
//! Construction is biased toward what ironclaw actually needs: stdio child
...
mod tests {
    ...
    async fn connect_stdio_missing_command_is_transport_error() {
        let err = McpClient::connect_stdio(
            "/path/that/does/not/exist/ironclaw-mcp-test",
            std::iter::empty::<&str>(),
            HashMap::new(),
        )
        .await
