...
const HOST_PATH_FILE_ENV_OVERRIDE: &str = "IRONCLAW_HOST_PATH_FILE";
    ...
    let bytes = tokio::fs::read(&path).await.map_err(|err| {
        if err.kind() == std::io::ErrorKind::NotFound {
            ToolError::Internal(format!(
                "artifact_path: discovery file {} not found — the host has not written it for this session. If you're a development run, the operator can find your files under their `~/.local/share/ironclaw/data/sessions/` tree.",
                path.display()
            ))
        } else {
