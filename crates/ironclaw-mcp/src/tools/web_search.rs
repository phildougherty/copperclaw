...
//! 2. `IRONCLAW_WEB_SEARCH_PROVIDER` environment variable.
...
/// 2. `IRONCLAW_WEB_SEARCH_PROVIDER` env var if set.
...
pub fn resolve_provider(
    ...
    if let Some(name) = env.get("IRONCLAW_WEB_SEARCH_PROVIDER") {
        let p = Provider::parse(&name)?;
        ensure_key_present(p, env)?;
        return Ok(p);
    }
...
mod tests {
    ...
    fn resolve_provider_env_override_wins_over_autodetect() {
        let env = env_with(&[
            ...
            ("IRONCLAW_WEB_SEARCH_PROVIDER", "brave"),
        ]);
