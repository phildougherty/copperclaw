...
use ironclaw_channels_core::{TodoItemStatus, TodoList, TodoListItem, TODO_MAX_ITEMS};
...
/// [`TodoItemStatus`] from `ironclaw-channels-core`. The two enums
...
/// `text` per-item to [`ironclaw_channels_core::TODO_MAX_ITEM_TEXT_CHARS`]
    ...
    let wire_items: Vec<TodoListItem> = items
        ...
        .map(|it| {
            ...
            let text = if trimmed.chars().count()
                > ironclaw_channels_core::TODO_MAX_ITEM_TEXT_CHARS
            {
                let cap = ironclaw_channels_core::TODO_MAX_ITEM_TEXT_CHARS - 1;
...
mod tests {
    ...
    fn build_wire_list_truncates_overlong_item_text() {
        let long_text = "a".repeat(
            ironclaw_channels_core::TODO_MAX_ITEM_TEXT_CHARS + 50,
        );
