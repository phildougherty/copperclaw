...
/// label on the `copperclaw_stuck_inbound_apology_total` metric.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl ApologyReason {
    ...
    pub const fn metric_label(self) -> &'static str {
        match self {
            Self::PendingTooLong => copperclaw_metrics::STUCK_REASON_PENDING_TOO_LONG,
            Self::ContainerSpawnFailed => copperclaw_metrics::STUCK_REASON_CONTAINER_SPAWN_FAILED,
        }
    }
}
...
pub fn check(
    ...
    for row in rows {
        ...
        mark_tries_apology_sent(inbound_pool.conn_mut(), row.id)?;
        copperclaw_metrics::inc_stuck_inbound_apology(&agent_group_id_str, reason.metric_label());
        tracing::info!(
            target: "copperclaw_host_sweep",
            session = %session.id,
...
mod tests {
    ...
    fn container_spawn_failure_emits_apology() {
        ...
        assert_eq!(
            ...
            copperclaw_metrics::STUCK_REASON_CONTAINER_SPAWN_FAILED
        );
