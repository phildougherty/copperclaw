...
pub fn check_with_hook(
    ...
    session_id: &SessionId,
    now: DateTime<Utc>,
    before_reset: &mut dyn FnMut(copperclaw_types::MessageId),
) -> Result<Vec<MessageReset>, SweepError> {
...
fn atomic_reclaim_claim(
    conn: &mut Connection,
    message_id: copperclaw_types::MessageId,
    now: DateTime<Utc>,
    threshold_ms: i64,
    ...
    fn runner_finishes_turn_between_scan_and_reset_is_not_reset() {
        ...
        let r = check_with_hook(
            ...
            &mut |hooked_id| {
                ...
                copperclaw_db::tables::processing_ack::update_status(
                    pool.conn(),
                    msg,
                    ProcessingStatus::Done,
                )
        ...
        // not have deleted it.
        let claim = copperclaw_db::tables::processing_ack::get(
