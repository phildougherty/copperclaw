...
//! [`copperclaw_db::session::open_inbound`].
//!
...
impl CreateAgentHandler {
    ...
    pub(super) fn copy_parent_session_routing(
        ...
        let routing = match copperclaw_db::tables::session_routing::read(&parent_conn) {
            Ok(Some(r)) => r,
            Ok(None) => {
                warn!(
        ...
        if let Err(err) = copperclaw_db::tables::session_routing::write(&child_conn, &routing) {
            warn!(
                ?err,
    ...
    /// operator can still drive the child manually via `cclaw chat`.
    ...
    pub(super) fn write_inbound_payload(
        ...
        let conn = match copperclaw_db::session::open_inbound(&paths) {
            Ok(c) => c,
            Err(err) => {
                warn!(
                    ?err,
