...
//! safety helpers in `copperclaw-db::attachments` (no `..`, no leading dots,
...
impl Module for MountSecurityModule {
    ...
    async fn install(&self, _ctx: Arc<dyn ModuleContext>) -> Result<(), ModuleError> {
        ...
        // implement `Module` so it can be enumerated in `cclaw modules list`.
        Ok(())
    }
}
...
mod tests {
    ...
    fn root() -> PathBuf {
        PathBuf::from("/srv/copperclaw/data/sessions")
    }
