...
//!    [`MIN_ELAPSED`] (30s). A quick turn's answer never made the user
//!    wait, so there is nothing to relieve.
...
/// Turns shorter than this keep today's single terminal emit: a quick
/// answer never kept the user waiting, so a paced reveal would only
/// *delay* it. Read off the HUD's existing `started_at` clock via
/// [`super::hud::TaskHud::elapsed`].
pub(super) const MIN_ELAPSED: Duration = Duration::from_secs(30);

/// Final answers shorter than this (in `char`s) are emitted in one shot
/// even on a long, edit-capable turn — a couple of short lines don't
/// benefit from a progressive reveal and the extra edit rows aren't
/// worth it.
pub(super) const MIN_GROW_CHARS: usize = 280;

/// Target number of `char`s revealed per step. Chosen so a
/// multi-paragraph answer animates in a handful of edits rather than
/// dozens.
const STEP_CHARS: usize = 240;

/// Hard cap on reveal steps regardless of answer length, so a very long
/// answer can't spray the delivery loop with dozens of edit rows. The
/// final step always carries the full text.
const MAX_STEPS: usize = 6;

/// Wall-clock pacing between reveal edits. Staggers the row writes so
/// the host's ~1s delivery poll picks them up across several ticks and
/// the message visibly grows, rather than all edits landing in one poll
/// and collapsing to "final text appears at once".
...
mod tests {
    ...
    fn should_grow_requires_all_of_capability_elapsed_length() {
        ...
        // Sub-30s turn: never grow (byte-identical single emit).
