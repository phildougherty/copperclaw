...
use ironclaw_db::tables::messaging_group_agents::{list_for_mg as list_wirings, MessagingGroupAgent};
use ironclaw_db::tables::messaging_groups::get_by_platform;
use ironclaw_db::tables::sessions::{
    create as create_session, find_for_agent, CreateSession,
};
use ironclaw_db::tables::unregistered_senders::{
    upsert as upsert_unregistered, UpsertUnregisteredSender,
};
use ironclaw_modules::context::{
...
impl Router {
    ...
    /// host's `iclaw router stats` rendering.
    ...
    fn route_one(
        ...
        user_id: Option<ironclaw_types::UserId>,
...
mod tests {
    ...
    async fn sender_scope_pending_yields_pending_outcome() {
        ...
        let rows = ironclaw_db::tables::unregistered_senders::list(fx.router.central(), None)
            .unwrap();
    ...
    async fn dropped_messages_recorded_for_no_mg() {
        ...
        let dropped = ironclaw_db::tables::dropped_messages::list(router.central(), None).unwrap();
