//! rmcp `ServerHandler` for the copperclaw tool inventory.
//!
    ...
    pub fn new(ctx: Arc<dyn ToolContext>) -> Self {
        ...
        let info = ServerInfo {
            ...
            server_info: Implementation {
                name: "copperclaw-mcp".into(),
                version: env!("CARGO_PKG_VERSION").into(),
            },
            instructions: Some(
                "copperclaw agent container MCP server: messaging, scheduling, self-mod tools."
                    .into(),
            ),
        };
        Self {
...
impl ServerHandler for CopperclawServer {
...
/// Build a fully-configured `CopperclawServer` ready to be `serve`d on a
/// transport (e.g. `TokioChildProcess` for stdio).
///
/// ```ignore
/// use std::sync::Arc;
/// use rmcp::ServiceExt;
/// use copperclaw_mcp::{build_server, MockToolContext};
///
...
/// # })
/// ```
pub fn build_server(ctx: Arc<dyn ToolContext>) -> CopperclawServer {
    CopperclawServer::new(ctx)
}
...
mod tests {
    ...
    fn server_info_advertises_tools_capability() {
        ...
        assert_eq!(info.server_info.name, "copperclaw-mcp");
