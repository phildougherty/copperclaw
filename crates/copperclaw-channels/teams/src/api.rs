...
/// emoji on some Teams clients), looked up through the transcript
/// vocabulary ([`vocab::for_channel`] binds `teams` to
/// [`vocab::ASCII`], byte-identical to the literals this adapter
/// hardcoded before M22 A4).
...
pub fn build_adaptive_todo_list(list: &TodoList) -> Value {
    ...
    for item in list.items.iter().take(48) {
        ...
        // Teams theming, not transcript vocabulary).
