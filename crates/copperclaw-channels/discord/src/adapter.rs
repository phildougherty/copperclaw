...
mod tests {
    ...
    async fn deliver_card_posts_embed_and_components_returns_id() {
        use copperclaw_channels_core::{Card, CardButton, CardField};
        let server = MockServer::start().await;
        Mock::given(method("POST"))
            .and(path("/channels/c1/messages"))
    ...
    fn build_card_payload_button_chunking_above_five_per_row() {
        use copperclaw_channels_core::{Card, CardButton};
    ...
    fn discord_todo_list_sample() -> copperclaw_channels_core::TodoList {
