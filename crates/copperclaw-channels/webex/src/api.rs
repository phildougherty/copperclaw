...
/// under Webex's message-size ceiling. Mirrors Telegram's / Teams's
/// `ACTIVITY_MAX_STEPS`.
...
/// Card. Used by both the initial emit and the (Webex-faked) edit path —
/// `deliver_breadcrumb` always POSTs a fresh card and routes both
/// through [`build_adaptive_breadcrumb`].
///
/// Layout mirrors `build_adaptive_collapsible` / `build_adaptive_thinking`:
/// a low-profile, always-visible summary `TextBlock` (collapsed one-line
...
/// ("Show steps") button — the closest native disclosure-widget the
/// Adaptive Cards schema offers (Webex 1.2 supports it).
///
/// Each step is styled on its own line: ASCII status marker, **bold**
/// tool name, `` `monospace` `` detail, then the result blurb. Every
/// dynamic field is placed into the card as a JSON string value (via
/// `json!`) so untrusted tool input is inherently safe — no raw markup
/// is concatenated onto the wire.
///
...
mod tests {
    ...
    fn build_adaptive_breadcrumb_aggregate_detail_goes_in_as_json_string() {
        ...
        // concatenated into the wire markup), so `<`/`>`/`&`/`"` round-trip
        // verbatim inside the card and are escaped at the JSON
        ...
        // (and Webex's card sanitiser) handle safety downstream.
