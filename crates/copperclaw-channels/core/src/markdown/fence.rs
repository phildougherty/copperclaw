//! Fence-state tracking for the shared chat-text splitter.
//!
//! A fence-aware splitter (see [`super::split_into_chunks`]) must never
//! cut a chunk in the middle of a code fence: a split fence renders as
//! garbage on Telegram / Discord (the closing marker of the first half is
//! missing, so everything after it is swallowed into the code block).
...
//!
//! MIGRATION (M18/C5b): this logic previously lived in
//! `copperclaw-host-delivery/src/fence.rs` (built by C2 with an explicit
//! "the shared markdown renderer is expected to absorb this" note). It
//! moved here verbatim so the renderer — the one place canonical markdown
//! turns into per-platform flavor — also owns fence-aware splitting.
//! `host-delivery` now consumes [`super::split_into_chunks`] /
//! [`super::is_balanced`] instead of owning the scanner.
...
pub(crate) struct FenceSpan {
    ...
    /// fence runs unclosed to end-of-text). Read by [`is_balanced`]; kept
    /// unconditional so the scanner's output is self-describing.
...
/// i.e. the text parses with balanced fences. Callers (and the splitter's
/// unit table) use it to assert every emitted chunk is balanced.
