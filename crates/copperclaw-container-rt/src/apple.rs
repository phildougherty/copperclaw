...
/// `container ls --label copperclaw.install=<slug> --quiet --all`
pub(crate) fn list_orphans_args(install_slug: &str) -> Vec<String> {
    vec![
        ...
        "--quiet".into(),
        "--filter".into(),
        format!("label=copperclaw.install={install_slug}"),
    ]
}
...
pub(crate) fn write_build_context(spec: &ImageBuildSpec) -> Result<PathBuf, RtError> {
    ...
    let unique = format!("copperclaw-build-{}", spec.fingerprint());
    dir.push(unique);
...
mod tests {
    ...
    fn list_orphans_args_carries_label_filter() {
        ...
        assert!(a.iter().any(|s| s == "label=copperclaw.install=default"));
    }
    ...
    fn build_args_format() {
        ...
        let a = build_args(dir, "copperclaw:abc");
        assert_eq!(a, vec!["build", "--tag", "copperclaw:abc", "/tmp/x"]);
    }
