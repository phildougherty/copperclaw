//! Wire protocol for the `iclaw` Unix-domain-socket transport.
//!
//! See `PLAN.md` § 5.4. Newline-delimited JSON, one request and one response
//! per connection. Both the host (M5 socket server) and the `iclaw` binary
...
use ironclaw_types::{AgentGroupId, MessagingGroupId, SessionId};
use serde::{Deserialize, Serialize};
use std::io;
use tokio::io::{AsyncBufReadExt, AsyncWrite, AsyncWriteExt, BufReader};

/// A `iclaw` request. `kind: "call"` is the only variant; the enum is kept
...
/// A `iclaw` response. Matches `Request` by `id`.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(tag = "kind", rename_all = "kebab-case")]
pub enum Response {
