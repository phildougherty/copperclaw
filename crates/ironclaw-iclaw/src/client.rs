//! Unix-socket client for the `iclaw` wire protocol.
//!
//! The host (M5) exposes a Unix-domain-socket server at `data/iclaw.sock`
//! (mode `0o600`) speaking the framing defined in [`crate::protocol`]. The
//! [`IclawClient`] in this module is the canonical client implementation —
//! the `iclaw` binary uses it, and so does anything else that wants to drive
...
/// Errors returned by [`IclawClient::call`].
...
mod tests {
    ...
    fn temp_socket() -> (TempDir, PathBuf) {
        ...
        let path = dir.path().join("iclaw.sock");
        (dir, path)
    }
    ...
    async fn call_round_trip_ok() {
        ...
        let client = IclawClient::connect(path);
        let data = client
    ...
    async fn call_timeout_when_server_does_not_respond() {
        ...
        let client = IclawClient::connect(path).with_timeout(Duration::from_millis(50));
        let err = client
            .call("groups.list", json!({}), Caller::Host)
            .await
            .unwrap_err();
    ...
    #[test]
    fn accessors() {
        let p = PathBuf::from("/tmp/iclaw.sock");
        let c = IclawClient::connect(p.clone()).with_timeout(Duration::from_secs(3));
