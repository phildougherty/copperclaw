...
mod tests {
    ...
    fn plan_with_socket_name_only() {
        let plan = EmacsClientPlan::from_config(&cfg_with("emacsclient", Some("ironclaw"), None));
        assert_eq!(plan.base_args, vec!["-s".to_string(), "ironclaw".into()]);
    }
    ...
    fn to_command_includes_socket_args_and_e_sexp() {
        ...
        let cmd = plan.to_command("(ironclaw-pop-inbound)");
        let std_cmd = cmd.as_std();
