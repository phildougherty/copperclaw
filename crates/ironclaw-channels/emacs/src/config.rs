...
//!   "socket_name":            "ironclaw",
//!   "socket_dir":             "/run/user/1000/emacs",
//!   "poll_interval_ms":       500,
//!   "inbound_queue_sexp":     "(ironclaw-pop-inbound)",
//!   "outbound_sexp_template": "(ironclaw-deliver ${BUFFER_JSON} ${TEXT_JSON})",
//!   "default_buffer":         "*ironclaw*"
//! }
//! ```
//!
...
/// Default poll interval (milliseconds) between `ironclaw-pop-inbound`
/// invocations.
...
///
/// ```elisp
/// (defun ironclaw-pop-inbound ()
///   "Pop the next queued message for ironclaw, or return nil."
///   ;; returns nil OR
///   ;;   `(("buffer" . ,buf) ("text" . ,text) ("sender" . ,sender))
///   )
/// ```
pub const DEFAULT_INBOUND_QUEUE_SEXP: &str = "(ironclaw-pop-inbound)";
...
///
/// ```elisp
/// (defun ironclaw-deliver (buffer text)
...
mod tests {
    ...
    fn full_config_parses() {
        ...
        assert_eq!(c.socket_name.as_deref(), Some("ironclaw"));
