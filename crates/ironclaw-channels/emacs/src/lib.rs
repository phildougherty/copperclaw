...
//!
//! ## `ironclaw-pop-inbound`
//!
//! Returns the next queued inbound message, or `nil`. The adapter polls
//! this on a configurable interval.
//!
//! ```elisp
//! (defvar ironclaw--inbox nil
//!   "Queue of pending inbound messages for ironclaw.")
//!
//! (defun ironclaw-enqueue (buffer text &optional sender)
//!   "Queue (BUFFER TEXT SENDER) as an inbound message for ironclaw."
//!   (let ((entry `(("buffer" . ,buffer)
//!                  ("text"   . ,text))))
//!     (when sender
//!       (setq entry (append entry `(("sender" . ,sender)))))
//!     (setq ironclaw--inbox (append ironclaw--inbox (list entry)))))
//!
//! (defun ironclaw-pop-inbound ()
//!   "Pop and return the oldest queued inbound message, or nil."
//!   (when ironclaw--inbox
//!     (prog1 (car ironclaw--inbox)
//!       (setq ironclaw--inbox (cdr ironclaw--inbox)))))
//! ```
//!
//! ## `ironclaw-deliver`
//!
