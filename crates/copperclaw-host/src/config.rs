...
//! | Var | Default | Meaning |
//! |---|---|---|
//! | `COPPERCLAW_DATA_DIR` | `data` | Root of central DB + per-session files. |
//! | `COPPERCLAW_INSTALL_SLUG` | `default` | Label propagated to container runtime for orphan cleanup. |
//! | `COPPERCLAW_LOG` | `info` | `tracing-subscriber` env filter. |
//! | `COPPERCLAW_CCLAW_SOCKET` | `<data>/cclaw.sock` | Unix socket the `cclaw` client dials. |
//! | `COPPERCLAW_DEFAULT_PROVIDER` | unset | Provider injected into new sessions when no per-group override. |
//! | `COPPERCLAW_DEFAULT_MODEL` | unset | Default Anthropic model id. |
//! | `COPPERCLAW_CHANNELS` | `cli` | Comma-separated list of channels to initialize. |
//! | `COPPERCLAW_CHANNELS_CONFIG` | `{}` | JSON object keyed by channel type; per-channel `setup.config`. |
//! | `COPPERCLAW_CLI_FIFO` | `<install_root>/chat.fifo` | Named pipe the cli channel reads from. `cclaw chat` writes one message per line into this FIFO. When unset, defaults to `<install_root>/chat.fifo` if `COPPERCLAW_DATA_DIR` is set (`install_root` = parent of data dir), otherwise the cli channel falls back to reading from the host's own stdin (developer REPL mode). |
//! | `COPPERCLAW_CLI_LOG` | `<install_root>/chat.log` | Append-only log the cli channel writes outbound replies to; `cclaw chat` tails it. Same default-resolution rules as `COPPERCLAW_CLI_FIFO`. |
//! | `COPPERCLAW_SKILLS_DIR` | unset | Directory of `SKILL.md` skill bundles auto-loaded into the agent system prompt. |
//! | `COPPERCLAW_GROUPS_DIR` | unset | Directory under which per-group `<ag_id>/skills/` overrides live. |
//! | `COPPERCLAW_SKILLS_MODE` | `inline` | How skill bodies reach the agent: `inline` (full bodies in the system prompt — today's behaviour) or `callable` (compact name+description index in the prompt, bodies retrieved on demand via the `load_skill` MCP tool). |
//! | `COPPERCLAW_METRICS_ADDR` | unset | Enable the Prometheus `/metrics` endpoint. Accepts `host:port` or a bare port (auto-prefixed to `127.0.0.1:`). Off by default per the conservative-defaults tenet. |
//! | `COPPERCLAW_LOG_DIR` | unset | When set, fan tracing output to `<dir>/host.log.<date>` via `tracing-appender::rolling::daily` in addition to stderr. Default off keeps the legacy stderr-only behaviour. |
//! | `COPPERCLAW_DEFAULT_IMAGE_TAG` | unset | sha-pinned default image tag for sessions when the per-group `container_configs.image_tag` is unset. Written by `copperclaw-setup` after building the image. |
...
//! | `COPPERCLAW_WEB_SEARCH_PROVIDER` | unset | Default `web_search` provider (`tavily` \| `exa` \| `brave` \| `serpapi`). Auto-detected from set API keys when unset. |
...
//! | `COPPERCLAW_CODEX_BINARY` | `/usr/local/bin/codex` (runner default) | Absolute path to the Codex CLI inside the container. Consulted by the runner only when `provider == "codex"`. Forwarded into every spawn so the operator can swap binaries via `.env` + SIGHUP. |
//! | `COPPERCLAW_CODEX_ARGS` | `--json` (runner default) | Comma-separated extra args appended to every Codex spawn (e.g. `--json,--no-color`). Consulted by the runner only when `provider == "codex"`. |
//!
//! The `cli` channel is always implicitly known but is only initialized if it
//! appears in `COPPERCLAW_CHANNELS`. Unknown channel names log a warning and
...
pub enum HostConfigError {
    /// `COPPERCLAW_CHANNELS_CONFIG` did not parse as JSON.
    #[error("COPPERCLAW_CHANNELS_CONFIG is not valid JSON: {0}")]
    BadChannelsConfig(serde_json::Error),
    /// `COPPERCLAW_CHANNELS_CONFIG` was not a JSON object.
    #[error("COPPERCLAW_CHANNELS_CONFIG must be a JSON object")]
    ChannelsConfigShape,
}
...
pub struct HostConfig {
    ...
    /// Path to the local Unix socket the `cclaw` client dials.
    ...
    /// `container_config.image_tag` is unset. Set by `copperclaw-setup`
    ...
    /// on demand. Set via `COPPERCLAW_SKILLS_MODE=inline|callable`.
...
impl HostConfig {
    ...
    pub fn from_map(map: &HashMap<String, String>) -> Result<Self, HostConfigError> {
        ...
        let ncl_socket_path = map
            .get("COPPERCLAW_CCLAW_SOCKET")
            .map_or_else(|| data_dir.join("cclaw.sock"), PathBuf::from);
        let default_provider = map.get("COPPERCLAW_DEFAULT_PROVIDER").cloned();
        let default_model = map.get("COPPERCLAW_DEFAULT_MODEL").cloned();
        let default_image_tag = map
            .get("COPPERCLAW_DEFAULT_IMAGE_TAG")
            .cloned()
            .filter(|s| !s.is_empty());
        let skills_dir = map
            .get("COPPERCLAW_SKILLS_DIR")
            .cloned()
            .filter(|s| !s.is_empty())
            .map(PathBuf::from);
        let groups_dir = map
            .get("COPPERCLAW_GROUPS_DIR")
            .cloned()
            .filter(|s| !s.is_empty())
            .map(PathBuf::from);
        let skills_mode = crate::container_manager::SkillsMode::parse_or_default(
            map.get("COPPERCLAW_SKILLS_MODE").map(String::as_str),
        );

        let channels_list = map
            .get("COPPERCLAW_CHANNELS")
            .cloned()
            .unwrap_or_else(|| DEFAULT_CHANNELS.to_owned());

        let channels_config_raw = map
            .get("COPPERCLAW_CHANNELS_CONFIG")
            .cloned()
        ...
        // install-root default. The bridge is what makes `cclaw chat`
        // round-trip into the host without colliding with the host's
        // own terminal stdin.
        let cli_fifo = map
            .get("COPPERCLAW_CLI_FIFO")
            .cloned()
            .filter(|s| !s.is_empty());
        let cli_log = map
            .get("COPPERCLAW_CLI_LOG")
            .cloned()
            .filter(|s| !s.is_empty());
...
fn env_to_map() -> HashMap<String, String> {
    std::env::vars()
        .filter(|(k, _)| k.starts_with("COPPERCLAW_"))
        .collect()
}

/// Resolve the install root from `COPPERCLAW_DATA_DIR`. The install
/// layout `copperclaw-setup` writes places `data/` under the install
/// root, so the parent of `COPPERCLAW_DATA_DIR` is where `chat.fifo`
...
/// `COPPERCLAW_CHANNELS_CONFIG` blob wins; this only fills in the
...
///    (`$XDG_DATA_HOME/copperclaw/.env` on Linux,
///    `~/Library/Application Support/copperclaw/.env` on macOS), which
///    is where `copperclaw-setup` writes by default.
...
/// `copperclaw-setup`'s `default_data_dir_for` so the two binaries
...
mod tests {
    ...
    fn defaults_when_env_empty() {
        ...
        assert_eq!(cfg.ncl_socket_path, PathBuf::from("data/cclaw.sock"));
    ...
    fn explicit_socket_overrides_default() {
        let cfg =
            HostConfig::from_map(&m(&[("COPPERCLAW_CCLAW_SOCKET", "/tmp/some.sock")])).unwrap();
    ...
    fn channels_list_parses() {
        let cfg = HostConfig::from_map(&m(&[
            ("COPPERCLAW_CHANNELS", "cli, telegram , , slack"),
            (
                "COPPERCLAW_CHANNELS_CONFIG",
    ...
    fn cli_bridge_paths_default_from_install_root() {
        // COPPERCLAW_DATA_DIR=/srv/copperclaw/data → install_root=/srv/copperclaw
        let cfg = HostConfig::from_map(&m(&[
            ("COPPERCLAW_DATA_DIR", "/srv/copperclaw/data"),
            ("COPPERCLAW_CHANNELS", "cli"),
        ]))
        .unwrap();
        let cli = &cfg.channels[0];
        assert_eq!(cli.channel_type, "cli");
        assert_eq!(cli.config["fifo"], "/srv/copperclaw/chat.fifo");
        assert_eq!(cli.config["log"], "/srv/copperclaw/chat.log");
    }
    ...
    fn cli_bridge_paths_env_var_overrides_default() {
        let cfg = HostConfig::from_map(&m(&[
            ("COPPERCLAW_DATA_DIR", "/srv/copperclaw/data"),
            ("COPPERCLAW_CHANNELS", "cli"),
            ("COPPERCLAW_CLI_FIFO", "/tmp/x.fifo"),
            ("COPPERCLAW_CLI_LOG", "/tmp/x.log"),
        ]))
        .unwrap();
    ...
    fn cli_bridge_paths_channels_config_json_wins() {
        // Explicit `COPPERCLAW_CHANNELS_CONFIG` for cli must NOT be
    ...
    fn cli_bridge_paths_omitted_when_no_install_root() {
        ...
        let cfg = HostConfig::from_map(&m(&[("COPPERCLAW_CHANNELS", "cli")])).unwrap();
    ...
    fn empty_channels_list_yields_no_channels() {
        let cfg = HostConfig::from_map(&m(&[("COPPERCLAW_CHANNELS", "")])).unwrap();
    ...
    fn default_provider_and_model_passthrough() {
        let cfg = HostConfig::from_map(&m(&[
            ("COPPERCLAW_DEFAULT_PROVIDER", "claude"),
            ("COPPERCLAW_DEFAULT_MODEL", "claude-3-5"),
        ]))
        .unwrap();
    ...
    fn skills_dir_empty_is_none() {
        let cfg = HostConfig::from_map(&m(&[("COPPERCLAW_SKILLS_DIR", "")])).unwrap();
    ...
    fn skills_mode_parses_callable() {
        let cfg = HostConfig::from_map(&m(&[("COPPERCLAW_SKILLS_MODE", "callable")])).unwrap();
        assert_eq!(
            cfg.skills_mode,
    ...
    fn skills_mode_unknown_value_falls_back_to_inline() {
        let cfg = HostConfig::from_map(&m(&[("COPPERCLAW_SKILLS_MODE", "bogus")])).unwrap();
        assert_eq!(
            cfg.skills_mode,
            crate::container_manager::SkillsMode::Inline
        );
    ...
    fn install_slug_and_log_overrides() {
        let cfg = HostConfig::from_map(&m(&[
            ("COPPERCLAW_INSTALL_SLUG", "ci-rig"),
            ("COPPERCLAW_LOG", "debug"),
        ]))
        .unwrap();
    ...
    fn install_env_file_for_macos() {
        ...
        assert_eq!(
            p,
            PathBuf::from("/Users/u/Library/Application Support/copperclaw/.env")
        );
    }
    ...
    fn install_env_file_for_linux_fallback() {
        ...
        assert_eq!(p, PathBuf::from("/home/u/.local/share/copperclaw/.env"));
    }

    #[test]
    fn install_env_file_for_other_os() {
        let p = install_env_file_for(Path::new("/h"), "freebsd");
        assert_eq!(p, PathBuf::from("/h/.copperclaw/.env"));
    }
}
