...
pub struct ManagerConfig {
    ...
    /// `COPPERCLAW_DEFAULT_PROVIDER` or `"anthropic"` as a fallback.
    pub default_provider: String,
    /// Default model id.
    pub default_model: String,
    /// Default reasoning-effort tier (`low`/`medium`/`high`). Read from
    /// `COPPERCLAW_DEFAULT_EFFORT` at boot. `None` (or `Medium`) means
    ...
    pub default_effort: Option<copperclaw_types::Effort>,
    ...
    /// small. Set via `COPPERCLAW_SKILLS_MODE` at boot.
    ...
    /// toolkit`. Set via `COPPERCLAW_CONTAINER_GPU=1` (or `all`, `true`).
    ...
    /// `COPPERCLAW_*` settings through to the runner. Keys with empty
