//! Integration test for the `copperclaw start` / `stop` / `status` /
//! `logs` lifecycle. Drives the freshly-built `copperclaw` binary as a
//! subprocess so we exercise the real re-exec dance, not just the
//! library helpers.
//!
//! Skipped when the binary isn't built yet (e.g. `cargo test -p
//! copperclaw-host --lib` from a cold checkout). `cargo test --workspace`
...
/// Path to the freshly-built `copperclaw` binary that Cargo provides
/// to integration tests for the bin we're testing.
fn copperclaw_bin() -> &'static str {
    env!("CARGO_BIN_EXE_copperclaw")
}

/// Spawn `copperclaw <args>` with `COPPERCLAW_DATA_DIR=tmp` and capture
...
fn run(tmp: &std::path::Path, args: &[&str]) -> std::process::Output {
    Command::new(copperclaw_bin())
        ...
        .env("COPPERCLAW_DATA_DIR", tmp)
        .env("COPPERCLAW_CHANNELS", "")
        .env("COPPERCLAW_LOG", "warn")
        .env_remove("COPPERCLAW_CCLAW_SOCKET")
        .output()
        .expect("spawn copperclaw")
}
...
fn logs_when_no_log_file_errors_cleanly() {
    ...
    assert!(
        stderr.contains("log file") || stderr.contains("copperclaw.log"),
        "stderr: {stderr}",
    );
}
...
/// a container runtime configured because `copperclaw run` aborts on
...
fn start_then_stop_round_trip() {
    ...
    // Kick off `copperclaw start`. With `COPPERCLAW_CHANNELS=""` and no
    ...
    if !out.status.success() {
        ...
        panic!("copperclaw start failed: {stderr}");
    }
