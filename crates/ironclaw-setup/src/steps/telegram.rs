...
//! headless case the answers come from `IRONCLAW_SETUP_TELEGRAM_*`
...
/// `IRONCLAW_SETUP_TELEGRAM_CHAT_ID`.
...
pub fn run_pairing(
    ...
    let Some(token) = capture_token(prompt, out)? else {
        out.push(
            "Telegram pairing skipped — wire it later via `iclaw channel ...`.".to_string(),
        );
        return Ok(None);
    };
...
fn capture_chat_id(
    ...
    if !want_capture {
        out.push(
            "Skipping chat-id capture — you can wire one later with `iclaw messaging-groups ...`."
                .to_string(),
        );
        return Ok(None);
    }
...
fn push_success_footer(out: &mut Vec<String>, bot_handle: Option<&str>, chat_id: Option<i64>) {
    ...
    out.push(format!(
        "Telegram is wired. Run `ironclaw run && iclaw chat`, or just message your bot at{handle_hint} directly."
    ));
    if chat_id.is_some() {
        out.push(
    ...
    out.push("To unwire later: `iclaw wirings delete <id>`.".to_string());
}
