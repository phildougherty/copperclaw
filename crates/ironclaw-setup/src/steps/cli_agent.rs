//! Step 9 — verify the `iclaw` CLI agent is on `PATH`.
...
impl Step for CliAgentStep {
    ...
    fn description(&self) -> &'static str {
        "Verify the iclaw CLI agent is on PATH"
    }
    ...
    fn run(
        ...
        let present = binary_on_path("iclaw");
        cfg.env_report.has_ncl = present;
        let message = if present {
            "iclaw: found on PATH".to_string()
        } else {
            "iclaw: NOT found on PATH (the CLI channel will be unavailable until installed)".to_string()
        };
        Ok(StepResult {
...
mod tests {
    ...
    fn step_records_presence() {
        ...
        assert_eq!(cfg.env_report.has_ncl, binary_on_path("iclaw"));
    }
