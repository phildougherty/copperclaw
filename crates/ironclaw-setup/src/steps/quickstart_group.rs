    ...
    fn run(
        ...
        match cfg.first_channel.as_str() {
            ...
            "telegram" => {
                ...
                messages.push(format!(
                    "bootstrapped agent group `{name}` for telegram (wrote IRONCLAW_CHANNELS + IRONCLAW_CHANNELS_CONFIG to .env)"
                ));
                if let Some(chat_id) = outcome.chat_id {
                    ...
                    messages.push(
                        "no TELEGRAM_CHAT_ID captured at setup time; the bot will create an unregistered-sender row on first inbound — approve with `iclaw approvals approve --channel telegram --identity <chat_id>`".to_string(),
                    );
                }
            }
        ...
        // `iclaw chat` for debugging without re-running setup.
...
/// `IRONCLAW_SETUP_TELEGRAM_CHAT_ID`); the bot still works but the
...
pub struct TelegramBootstrap {
    ...
    pub messaging_group_id: Option<ironclaw_types::MessagingGroupId>,
    pub wiring_id: Option<ironclaw_types::WiringId>,
}
...
pub fn bootstrap_telegram_install(
    ...
    //    IRONCLAW_CHANNELS to know which factories to call; the cli
    //    channel stays in the list so `iclaw chat` keeps working as a
    //    debugging fallback.
    append_env_var(&env_path, "IRONCLAW_CHANNELS", "cli,telegram")?;

    // 2. The telegram factory itself reads bot_token from the
    //    IRONCLAW_CHANNELS_CONFIG JSON, not from TELEGRAM_BOT_TOKEN
    ...
    append_env_var(&env_path, "IRONCLAW_CHANNELS_CONFIG", &channels_config)?;
    ...
    //    inbound (operator approves via `iclaw approvals approve ...`).
...
fn read_env_var(path: &Path, key: &str) -> Result<Option<String>, StepError> {
    ...
    for line in contents.lines() {
        if let Some(rest) = line.strip_prefix(&prefix) {
            ...
            // dotenvy-quoted (e.g. IRONCLAW_CHANNELS_CONFIG='{"..."}').
            let trimmed = rest
                .strip_prefix('\'')
...
mod tests {
    ...
    fn bootstrap_telegram_install_writes_env_vars_and_db_rows_with_chat_id() {
        ...
        // edit .env + run 3 `iclaw` commands by hand.
        ...
        let cfg = SetupConfig {
            ...
            central_db_path: dir.path().join("ironclaw.db"),
        ...
        assert!(
            env.contains("IRONCLAW_CHANNELS=cli,telegram"),
            "missing channels list:\n{env}"
        );
        assert!(
            env.contains("IRONCLAW_CHANNELS_CONFIG='"),
    ...
    fn bootstrap_telegram_install_without_chat_id_still_enables_channel() {
        ...
        let env = std::fs::read_to_string(&env_path).unwrap();
        assert!(env.contains("IRONCLAW_CHANNELS=cli,telegram"));
        assert!(env.contains("IRONCLAW_CHANNELS_CONFIG='"));
    }
