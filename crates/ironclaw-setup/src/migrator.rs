...
//! existing `ironclaw.db` from the supplied directory into the new data directory
...
/// migrations. Idempotent and safe even when no `ironclaw.db` is present at
/// `source_dir`.
pub fn migrate_from(source_dir: &Path, dest_dir: &Path) -> Result<MigrationOutcome, MigratorError> {
    ...
    let central_db_path = dest_data.join("ironclaw.db");

    let source_db = source_dir.join("data").join("ironclaw.db");
...
mod tests {
    ...
    fn migrate_from_missing_source_errors() {
        ...
        let err =
            migrate_from(Path::new("/definitely/does/not/exist/ironclaw"), dest.path()).unwrap_err();
    ...
    fn migrate_from_copies_existing_db() {
        ...
        let src_db = src_data.join("ironclaw.db");
