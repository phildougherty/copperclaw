...
//! - [`EnvBacked`]: reads answers from `IRONCLAW_SETUP_*` env vars.
...
/// to `IRONCLAW_SETUP_<KEY>`. Keys must be `SCREAMING_SNAKE_CASE`.
pub trait Prompt {
...
mod tests {
    ...
    fn env_var_name_is_prefixed() {
        assert_eq!(EnvBacked::var_name("DATA_DIR"), "IRONCLAW_SETUP_DATA_DIR");
    }

    #[test]
    fn env_input_uses_env_when_present() {
        let mut env = HashMap::new();
        env.insert("IRONCLAW_SETUP_DATA_DIR".into(), "/srv/x".into());
    ...
    fn env_input_errors_without_default() {
        ...
        match err {
            PromptError::Missing(name) => assert_eq!(name, "IRONCLAW_SETUP_MISSING"),
    ...
    fn env_confirm_parses_true_variants() {
        for v in ["1", "true", "TRUE", "yes", "Y", "on"] {
            let mut env = HashMap::new();
            env.insert("IRONCLAW_SETUP_X".into(), v.into());
            let p = EnvBacked::with_env(env);
    ...
    fn env_confirm_garbage_is_parse_error() {
        let mut env = HashMap::new();
        env.insert("IRONCLAW_SETUP_X".into(), "maybe".into());
        let p = EnvBacked::with_env(env);
    ...
    fn env_secret_reads_value() {
        let mut env = HashMap::new();
        env.insert("IRONCLAW_SETUP_TOKEN".into(), "sk-123".into());
        let p = EnvBacked::with_env(env);
    ...
    fn prompt_error_display_missing() {
        let e = PromptError::Missing("IRONCLAW_SETUP_X".into());
        assert!(e.to_string().contains("IRONCLAW_SETUP_X"));
    }
    ...
    fn env_backed_drives_full_step_set() {
        ...
        let mut env = HashMap::new();
        env.insert("IRONCLAW_SETUP_DATA_DIR".into(), "/srv/x".into());
        env.insert("IRONCLAW_SETUP_BUILD_IMAGE".into(), "no".into());
        env.insert("IRONCLAW_SETUP_USE_ONECLI".into(), "no".into());
        env.insert("IRONCLAW_SETUP_ANTHROPIC_API_KEY".into(), "sk-test".into());
        env.insert("IRONCLAW_SETUP_MOUNTS".into(), String::new());
        env.insert("IRONCLAW_SETUP_WRITE_SERVICE_UNIT".into(), "no".into());
        env.insert("IRONCLAW_SETUP_TIMEZONE".into(), "Etc/UTC".into());
        env.insert("IRONCLAW_SETUP_FIRST_CHANNEL".into(), "cli".into());
        let p = EnvBacked::with_env(env);
