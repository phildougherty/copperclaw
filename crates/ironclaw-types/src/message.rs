...
pub enum MessageKind {
    ...
    /// `Breadcrumb` payload (see `ironclaw_channels_core::Breadcrumb`).
    ///
    ...
    /// `TodoList` payload (see `ironclaw_channels_core::TodoList`).
    ///
    ...
    /// `ironclaw_channels_core::DiffCard`).
    ///
    ...
    /// `ErrorCard` payload (see `ironclaw_channels_core::ErrorCard`)
    /// in `content.error`.
    ///
    ...
    ///    row — operators still see the row in `iclaw
    ...
    /// `ironclaw_channels_core::ThinkingBlock`) in `content.thinking`.
    ///
    ...
    /// via `iclaw groups config edit <id>`.
    ///
