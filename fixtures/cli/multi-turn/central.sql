-- Replay fixture seed: cli/multi-turn.
--
-- Identical wiring to fixtures/cli/text-reply: one agent group bound to
-- the cli/stdin messaging group, engage pattern `.*`, session mode
-- `shared` so both inbound events route to the same session.
