-- Replay fixture seed: cli/empty-llm-response.
--
-- Same shape as cli/text-reply: one agent group "Replay" wired to a
-- single cli/stdin messaging group via a pattern-match wiring. The
-- only thing that varies vs text-reply is the LLM response (no
-- content blocks) — the routing topology stays identical so the
-- diff highlights only the empty-content behaviour.
