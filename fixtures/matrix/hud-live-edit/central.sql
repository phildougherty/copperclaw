-- Replay fixture seed: matrix/hud-live-edit (M19 F1).
--
-- Single agent group "Replay" wired to a single matrix messaging group
-- keyed on room id "!a:m.org" (the same room id used in
-- copperclaw-channels-matrix's parse unit tests). Engage mode `pattern`
-- with `.*` so every inbound matches; session mode `shared`.
--
-- Matrix is in `EDIT_CAPABLE_CHANNELS` (M19 F1 reconciled its trait
-- `edit_message` with that promise), so the runner's Task HUD resolves
-- to the live self-editing behaviour on this channel. The fixture drives
-- a one-tool turn so the HUD posts a chip and then edits it in place.
