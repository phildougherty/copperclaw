-- Replay fixture seed: slack/inbound-file-attachment.
--
-- Single agent group "Replay" wired to a single slack messaging group
-- keyed on channel id "C1" (the same channel id used in
-- copperclaw-channels-slack's events router unit tests). Engage mode
-- `pattern` with `.*` so every inbound matches; session mode `shared` so
-- both steps land in one session.
