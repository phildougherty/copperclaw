-- Replay fixture seed: telegram/slash-* (M18 R1 slash commands).
--
-- Single agent group "Replay" wired to a telegram GROUP chat (chat_id
-- "100", is_group = 1) with engage mode `mention` and NO pattern: an
-- unmentioned plain message would be mention-gated, so the ONLY reason
-- a bare slash command routes is the router's command bypass.
