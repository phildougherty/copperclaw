-- Replay fixture seed: telegram/inbound-text-message.
--
...
-- crate uses in its unit tests). Engage mode `pattern` with `.*` so
-- every inbound matches; session mode `shared` so all turns land in
-- one session.
