-- Replay fixture seed: telegram/inbound-document-attachment.
--
-- Single agent group "Replay" wired to a single telegram messaging
-- group keyed on chat_id "100" (the same chat_id the telegram ingress
-- crate uses in its own unit tests). Engage mode `pattern` with `.*` so
-- every inbound matches; session mode `shared` so both steps land in
-- one session.
