-- Replay fixture seed: telegram/auto-compaction (M22 Wave 0).
--
...
--
-- Three plain-text inbounds, no tool calls: the first two exchanges
-- grow the persisted history past the manifest's soft compaction
-- target, so the third inbound's run_loop iteration auto-compacts
-- (one scripted summary call) before its turn. No tools run, so the
-- Task HUD posts nothing and the streams stay focused on compaction.
